...
- `apps/mobile/src/config/ritualsConfig.ts` — единственный мост HA ↔ domain
...
- `config/rituals.default.yaml` — источник правды по entity_id
...
- Хардкодить `entity_id` в коде — только через `loadRitualsConfig()`.
...
6. **Подписки.** Только `entity_id` из `loadRitualsConfig()`, а не весь HA.
...
- [ ] `entity_id` не утёк за пределы `src/ha/**` и `ritualsConfig.ts`.
