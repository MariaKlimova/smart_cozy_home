...
description: >-
  ...
  категорийные Findings, «§3.5 проход» — без сжатия в prose.
model: inherit
readonly: true
---
...
- Перед финальным ответом: заполнены «Все findings» + все категорийные таблицы;
  счётчики Сводки сходятся (см. §6 «Самопроверка»).
...
### Анти-сжатие (нарушение = неполный ревью)

Финальный ответ — **единственный** deliverable ревью. Запрещено:

- Сжимать findings в prose / bullet-list / «ключевые N пунктов» вместо строк таблицы.
- Опускать строки таблицы «для краткости», «см. выше», «и ещё N».
- Подменять полные таблицы одной «Сводкой».
- Писать finding только в `user_visible_high_level_summary` / коротком summary —
  summary (если есть) ≤2 предложения; **все** таблицы — только в финальном
  markdown-теле ответа.
- Объединять несколько разных проблем в одну строку таблицы.

Каждый finding = **ровно одна строка** в нужной категорийной таблице **и**
ровно одна строка в «Все findings». Пустая категория = «—», не пустой заголовок
без тела.

### Самопроверка перед отправкой (обязательно)

Перед финальным ответом сверь числа (если не сходится — допиши строки, не
правь счётчики «на глаз»):

1. `Сводка.Blocking` == число строк severity=blocking во «Все findings»
2. `Сводка.Recommendation` == число строк категории Recommendation
   (только non-§3.5; без copy/tokens/duplication/dead/clean)
3. `Сводка.Duplication` / `Dead code` / `Clean code` == число строк в
   соответствующих категорийных таблицах
4. Число строк «Все findings» == сумма строк всех категорийных таблиц
   Findings (Blocking + Recommendation + Hardcoded copy + Hardcoded tokens +
   Duplication + Dead code + Clean code); пустые «—» = 0
5. Число строк «§3.5 проход» == числу кандидатов Шага A (или 1 строка
   «кандидатов нет»)
6. Каждый blocking из copy/tokens/duplication/dead/clean **есть** и во
   «Все findings», и в своей категорийной таблице (не только в Blocking)
...
Единый индекс **всех** замечаний (blocking + recommendation из всех категорий).
Без этой таблицы ревью неполное. Колонка ID сквозная: `B1`, `B2`, … для
blocking; `R1`, `R2`, … для recommendation.

| ID  | Severity | Категория | Файл | Строка | Проблема (кратко) | Рекомендация (кратко) |
| --- | -------- | --------- | ---- | ------ | ----------------- | --------------------- |
| B1  | blocking | Dead code / Hardcoded tokens / Duplication / Clean code / Blocking / Hardcoded copy | `path` | ~N | ... | ... |
| R1  | recommendation | Recommendation / Duplication / Dead code / Clean code | `path` | ~N | ... | ... |

(если findings нет — одна строка «—» во всех ячейках.)

### Findings

Ниже — те же findings **по категориям** (полный текст проблемы/рекомендации).
ID должны совпадать с «Все findings». Нельзя иметь finding только здесь или
только в индексе.

#### Blocking

Только findings категории «прочее blocking» (архитектура, секреты, БЭМ-структура,
recovery/мусор в PR, сломанный контракт), которых **нет** в copy / tokens /
duplication / dead / clean. ID вида `B*`.
...
Только non-§3.5 recommendation, которых **нет** в duplication / dead / clean.
ID вида `R*`.
...
| ID  | Файл   | Строка | Литерал | Предложенный ключ в `copy` |
| --- | ------ | ------ | ------- | -------------------------- |
| B2  | `path` | ~N     | `'…'`   | `copy.section.key`         |

(если нет — «—». Каждый пункт — **blocking**, ID `B*`, строка также во «Все findings».)

#### Hardcoded tokens

| ID  | Файл   | Строка | Литерал | Token / паттерн |
| --- | ------ | ------ | ------- | --------------- |
| B3  | `path` | ~N     | `#FFF` / `20` | `c.surface` / `spacing.lg` |

(если нет — «—». Каждый пункт — **blocking**, ID `B*`, строка также во «Все findings».)

#### Duplication

| ID  | Severity | Файл (дубль) | Дублирует | Строка | Куда вынести |
| --- | -------- | ------------ | --------- | ------ | ------------ |
| B4 / R2 | blocking / recommendation | `path` | `path` | ~N | ... |

(если нет — «—». Blocking-дубли учитывай в счётчике Blocking; строка также во «Все findings».)

#### Dead code

| ID  | Severity | Файл   | Символ / блок | Проблема | Действие |
| --- | -------- | ------ | ------------- | -------- | -------- |
| B5 / R3 | blocking / recommendation | `path` | `fooBar` | export без imports | удалить или подключить |

(если нет — «—». Blocking-пункты учитывай в счётчике Blocking; строка также во «Все findings».)

#### Clean code

| ID  | Severity | Файл   | Строка | Проблема (SRP / имя / проза / coupling / boy scout) | Рекомендация |
| --- | -------- | ------ | ------ | ---------------------------------------------------- | ------------ |
| B6 / R4 | blocking / recommendation | `path` | ~N | ... | ... |
...
findings §3.5, не число кандидатов. Строка также во «Все findings».
...
| -------- | --------------- | --- | --- | ----- | -------- | --------- | ------------ |
| `path`   | новый / ≥20 LOC / God-риск | ok / finding B6 | ok / finding R4 | ok / … | ok / … | ok / … | да — кратко / — |
...
В ячейках ok/finding указывай ID из «Все findings», не номер внутри категории.)
...
Заголовки и порядок секций менять нельзя. Секция «Все findings» обязательна
всегда (даже если там «—»).
