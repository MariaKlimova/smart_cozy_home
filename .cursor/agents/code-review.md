...
10. **Чистота кода (clean code)** — см. раздел 3.5; применяй к новым/существенно изменённым модулям в diff.
...
После diff **обязательно** оцени чистоту **новых и существенно изменённых**
модулей (хуки, lib, mappers, экраны с выносом логики). Цель — не стиль ради
стиля, а то, что ломает сопровождение: God-объекты, скрытые side-effects,
нечитаемые цепочки, мёртвый шум.

Правила не заменяют §3.1–3.4: дубли, мёртвый код, copy/tokens — отдельно.
Здесь — структура и ясность кода в зоне diff.

### Алгоритм

1. **Single Responsibility** — один файл/функция/хук ≈ одна задача.
   - God-hook / God-компонент (load + write + queue + UI-state в одном) без
     явной оркестрации → **recommendation** (или **blocking**, если PR
     *добавляет* новую зону ответственности в уже раздутый модуль без выноса).
   - `utils.ts` / `helpers.ts` как свалка разнородного → **recommendation**:
     разнести по именам, отражающим задачу.
2. **Имя = поведение** — `load*` не должен молча писать в HA; `refresh` не
   должен скрывать второй side-effect без JSDoc. Несоответствие имени и
   side-effects → **recommendation** (blocking, если это вводит баг/ложный
   контракт для вызывающего кода).
3. **Читаемость сверху вниз («как проза»)** — цепочки из 3+ внутренних шагов
   (`enqueue → persist → patch`) должны жить в одном модуле или читаться без
   прыжков по 4 файлам. Копипаста почти одинаковых сеттеров (number/boolean/color)
   без общего helper → **recommendation** (§3.1 тоже).
4. **Жёсткая связанность** — хардкод ключей полей / entity / device id,
   размазанный по 2+ файлам одного feature; скрытая зависимость от чужого store
   (например bedroom внутри scenarios) без явной точки входа → **recommendation**.
5. **Boy scout в рамках diff** — если PR трогает модуль и рядом лежит явный
   мёртвый export / one-liner wrappers без пользы / устаревший комментарий,
   отметь **recommendation** «почистить заодно» (не требуй рефакторить весь файл
   вне scope).
6. **Не придирайся** к осознанным trade-offs: очередь записей, silent catch
   после успешного write, тонкий `useCallback` для стабильной ссылки, публичный
   фасад хука поверх внутренних helpers. Отметь в «Что проверено и чисто».
...
| #   | Severity | Файл   | Строка | Проблема (SRP / имя / проза / coupling) | Рекомендация |
| --- | -------- | ------ | ------ | --------------------------------------- | ------------ |
| 1   | blocking / recommendation | `path` | ~N | ... | ... |

(если нет — «—». Blocking-пункты учитывай в счётчике Blocking; recommendation — в Clean code и Recommendation по смыслу, в сводке Clean code = число findings §3.5.)
