...
import { buildSleepMetricYDomain } from './buildSleepMetricChartData';

describe('buildSleepMetricYDomain', () => {
  it('uses default co2 scale when all points are within 300–1200', () => {
    const domain = buildSleepMetricYDomain(
      [
        { x: 0, y: 650 },
        { x: 0.5, y: 1100 },
        { x: 1, y: 720 },
  ...
  it('expands co2 max with 50 padding for a single high point', () => {
    const domain = buildSleepMetricYDomain([{ x: 0, y: 1250 }], 'co2');

    assert.deepEqual(domain, { min: 300, max: 1300 });
  });

  it('expands co2 min below 300 when data is very low', () => {
    const domain = buildSleepMetricYDomain([{ x: 0, y: 280 }], 'co2');
