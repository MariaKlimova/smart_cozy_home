...
- Повторный тап по active-режиму в приложении → `script.exit_home_mode` (`home_mode = none`, `script.turn_off` на scripts режимов, стоп музыки). Тот же script вызывается голосом («выключи режим»).
- Запуск другого режима, пока previous active → приложение сначала `script.turn_off` на script предыдущего режима, затем `script.turn_on` нового.
...
JSON-схема (v1). В HA helper хранится **компактный** вид (лимит `input_text` = 255):
...
Эквивалент с объектами `{ "enabled", "time" }` приложение ещё читает (legacy). Пишет только компактный формат.
...
## Параметры сценариев (input_number / input_boolean / input_text)

Источник правды для значений — HA. Приложение читает state и пишет через `input_number.set_value`, `input_boolean.turn_on/off`, `input_text.set_value`.

| Сценарий | Helpers |
|----------|---------|
| **evening** | `input_number.evening_brightness` (15), `input_number.evening_temperature` (18), `input_boolean.evening_curtains` (true), `input_boolean.evening_humidifier` (true), `input_text.evening_playlist` ("") |
| **sleep** | `input_number.sleep_temperature` (17), `input_boolean.sleep_window` (false), `input_boolean.sleep_nightlight` (true), `input_number.sleep_nightlight_brightness` (8), `input_text.sleep_nightlight_color` (`{"rgb_color":[242,145,61]}`), `input_text.sleep_playlist` ("") |
| **morning** | `input_number.morning_brightness` (80), `input_number.morning_warmup_minutes` (20), `input_text.morning_playlist` ("") |
...
| **cozy** | `input_number.cozy_brightness` (40), `input_number.cozy_temperature` (21), `input_text.cozy_playlist` ("") |
| **focus** | `input_number.focus_brightness` (90), `input_number.focus_temperature` (19), `input_text.focus_playlist` ("") |

**Плейлист (SH-58):** пустая строка = остановить музыку на станции (не оставлять прошлый плейлист). Сценарии с плейлистом вызывают `script.play_bedroom_playlist` → play или `script.stop_bedroom_music`. Away / coming_home / `script.exit_home_mode` всегда стопают музыку. Тип `text` у Yandex.Station — TTS, не команда. Голосовой запуск — `yandex_station_intents` → `script.turn_on` (см. `packages/ha-installer/yandex_station_intents.yaml`).

**Калибровка устройства (не сценарий):** `input_number.bedroom_light_visible_min` (0, max 99) — порог «свет виден с». Логические % слайдера и `*_brightness` пересчитываются в железо `[min, 100]` через `script.bedroom_set_light_logical` и в приложении (`HA_ENTITIES.devices.lightVisibleMin` / `mapLogicalToDevicePct`).
...
**Спальня:** `light.bedroom`, `input_number.bedroom_light_visible_min` (калибровка яркости), `light.bedroom_nightlight`, `cover.bedroom_curtains`, `cover.bedroom_window`, `climate.bedroom_ac`, `climate.bedroom_ventilation`, `climate.bedroom_radiator`, `humidifier.bedroom` (основной увлажнитель) / `switch.bedroom_humidifier` (фолбек через розетку), `sensor.bedroom_co2`, `sensor.bedroom_temperature`, `sensor.bedroom_humidity`, `sensor.bedroom_pressure` (значение в **mmHg / мм рт. ст.**, не hPa — приложение показывает число как есть), `binary_sensor.bedroom_occupancy`, `media_player.bedroom_station` (опционально, плейлисты сценариев)
