...
- Повторный тап по active-режиму в приложении → `home_mode = none` и `script.turn_off` на script этого режима (чтобы остановить длинный sequence, напр. morning ramp).
...
| **sleep** | `input_number.sleep_temperature` (17), `input_boolean.sleep_window` (false), `input_boolean.sleep_nightlight` (true), `input_number.sleep_nightlight_brightness` (8), `input_text.sleep_nightlight_color` (`{"rgb_color":[242,145,61]}`) |
...
**Спальня:** `light.bedroom`, `input_number.bedroom_light_visible_min` (калибровка яркости), `light.bedroom_nightlight`, `cover.bedroom_curtains`, `cover.bedroom_window`, `climate.bedroom_ac`, `climate.bedroom_ventilation`, `climate.bedroom_radiator`, `humidifier.bedroom` (основной увлажнитель) / `switch.bedroom_humidifier` (фолбек через розетку), `sensor.bedroom_co2`, `sensor.bedroom_temperature`, `sensor.bedroom_humidity`, `sensor.bedroom_pressure` (значение в **mmHg / мм рт. ст.**, не hPa — приложение показывает число как есть), `binary_sensor.bedroom_occupancy`
